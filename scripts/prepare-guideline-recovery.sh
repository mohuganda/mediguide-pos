...
usage() {
  printf '%s\n' \
    ...
    '  MEDIGUIDE_REPORT_TOKEN        token with guideline.review (never printed)' \
    '  MEDIGUIDE_MARKDOWN_EDIT_TOKEN token with guideline.markdown.edit for --execute' \
    '  MEDIGUIDE_REGENERATION_TOKEN  token with guideline.structure.regenerate for --execute' \
    '' \
    'For a local all-in-one administrator only, MEDIGUIDE_ACCESS_TOKEN can be used as' \
    'a fallback for any token above. Production should use separated role tokens.' \
    '' \
    'Usage:' \
...
common_token="${MEDIGUIDE_ACCESS_TOKEN:-}"
report_token="${MEDIGUIDE_REPORT_TOKEN:-$common_token}"
markdown_edit_token="${MEDIGUIDE_MARKDOWN_EDIT_TOKEN:-$common_token}"
regeneration_token="${MEDIGUIDE_REGENERATION_TOKEN:-$common_token}"
if [[ -z "${MEDIGUIDE_API_URL:-}" || -z "$report_token" ]]; then
  printf 'MEDIGUIDE_API_URL and MEDIGUIDE_REPORT_TOKEN are required.\n' >&2
  exit 2
fi
if [[ "$execute" == true && ( -z "$markdown_edit_token" || -z "$regeneration_token" ) ]]; then
  printf 'MEDIGUIDE_MARKDOWN_EDIT_TOKEN and MEDIGUIDE_REGENERATION_TOKEN are required with --execute.\n' >&2
  exit 2
fi
...
report_authorization="Authorization: Bearer ${report_token}"
markdown_edit_authorization="Authorization: Bearer ${markdown_edit_token}"
regeneration_authorization="Authorization: Bearer ${regeneration_token}"
source_report="$(curl --fail-with-body --silent --show-error -H "$report_authorization" "$api_url/api/v2/guideline-versions/$source_version_id/completeness-report")"
...
duplicate_response="$(curl --fail-with-body --silent --show-error -X POST -H "$markdown_edit_authorization" -H 'Content-Type: application/json' --data "$duplicate_payload" "$api_url/api/v2/guideline-versions/$source_version_id/duplicate")"
...
regenerate_response="$(curl --fail-with-body --silent --show-error -X POST -H "$regeneration_authorization" -H 'Content-Type: application/json' --data "$regenerate_payload" "$api_url/api/v2/guideline-versions/$candidate_version_id/regenerate")"
...
while (( SECONDS < deadline )); do
  job_response="$(curl --fail-with-body --silent --show-error -H "$regeneration_authorization" "$api_url/api/v2/guideline-versions/$candidate_version_id/regeneration-jobs/$job_id")"
...
curl --fail-with-body --silent --show-error -H "$report_authorization" "$api_url/api/v2/guideline-versions/$candidate_version_id/completeness-report/export?format=json" -o "$report_path"
