...
import { AlertTriangle, CheckCircle2, Download, Eye, FileSearch, FileText, FileUp, History, Pencil, Plus, RefreshCw, Trash2 } from "lucide-react"
...
import { outbreaksService, type OutbreakAuditRecord, type OutbreakDocumentContent, type OutbreakDocumentInput, type OutbreakDocumentRecord, type OutbreakDocumentSearchPreview } from "@/services/outbreaks.service"
...
export function OutbreakDocumentsWorkspace({ outbreakId, initialDocumentId }: { outbreakId: string; initialDocumentId?: string }) {
  ...
  const [preview, setPreview] = React.useState<OutbreakDocumentContent | null>(null)
  const [searchPreview, setSearchPreview] = React.useState<OutbreakDocumentSearchPreview | null>(null)
  ...
  async function showPreview(item: OutbreakDocumentRecord) {
    if (!item.id) return
    setWorkingId(item.id)
    try { setPreview(await outbreaksService.documentContent(outbreakId, item.id)) }
    catch (value) { showToast.error("Preview unavailable", message(value)) }
    finally { setWorkingId(null) }
  }

  async function reprocess(item: OutbreakDocumentRecord) {
    if (!item.id || item.lock_version === undefined || !window.confirm("Rebuild the search index and safe preview from the stored immutable file?")) return
    setWorkingId(item.id)
    try { await outbreaksService.reprocessDocument(outbreakId, item.id, item.lock_version); await refresh(); showToast.success("Document reprocessed", "Search and preview status were rebuilt from the checksum-verified managed file.") }
    catch (value) { showToast.error("Reprocessing failed", message(value)) }
    finally { setWorkingId(null) }
  }

  async function previewSearch(item: OutbreakDocumentRecord) {
    if (!item.id) return
    const query = window.prompt("Enter a phrase to preview against the derived search content")?.trim()
    if (!query || query.length < 2) return
    setWorkingId(item.id)
    try { setSearchPreview(await outbreaksService.documentSearchPreview(outbreakId, item.id, query)) }
    catch (value) { showToast.error("Search preview unavailable", message(value)) }
    finally { setWorkingId(null) }
  }
  ...
  return <Card>
    ...
    <CardContent className="space-y-4">
      ...
      {!documents.length ? <p className="text-sm text-muted-foreground">No outbreak documents have been created.</p> : <div className="space-y-3">{documents.map(item => <DocumentRow key={item.id} item={item} busy={workingId === item.id} onEdit={() => edit(item)} onUpload={file => void upload(item, file)} onTransition={action => void transition(item, action)} onDelete={() => void remove(item)} onAudit={() => void showAudit(item)} onPreview={() => void showPreview(item)} onSearchPreview={() => void previewSearch(item)} onReprocess={() => void reprocess(item)} />)}</div>}
      {preview ? <div className="space-y-3 rounded-md border p-4"><div className="flex items-center justify-between gap-3"><div><h3 className="font-semibold">{preview.title}</h3><p className="text-xs text-muted-foreground">Read-only {label(preview.format)} preview derived from the managed file.</p></div><Button size="sm" variant="ghost" onClick={() => setPreview(null)}>Close</Button></div>{preview.format === "markdown" ? <MarkdownPreview content={preview.content || ""} /> : <pre className="max-h-[640px] overflow-auto whitespace-pre-wrap rounded-md bg-muted p-4 text-sm">{preview.content}</pre>}</div> : null}
      {searchPreview ? <div className="space-y-2 rounded-md border p-4"><div className="flex items-center justify-between"><h3 className="font-semibold">Search preview: “{searchPreview.query}”</h3><Button size="sm" variant="ghost" onClick={() => setSearchPreview(null)}>Close</Button></div><p className="text-xs text-muted-foreground">Index status: {label(searchPreview.search_index_status)}{searchPreview.indexed_at ? ` · last indexed ${new Date(searchPreview.indexed_at).toLocaleString()}` : ""}</p>{!searchPreview.searchable ? <p className="text-sm text-amber-700">This document is not currently discoverable in public search.</p> : null}{searchPreview.matching_heading ? <p className="text-sm font-medium">Matching section: {searchPreview.matching_heading}</p> : null}<p className="rounded-md bg-muted p-3 text-sm">{searchPreview.snippet || "No derived-content match found."}</p></div> : null}
...
function DocumentRow({ item, busy, onEdit, onUpload, onTransition, onDelete, onAudit, onPreview, onSearchPreview, onReprocess }: { item: OutbreakDocumentRecord; busy: boolean; onEdit: () => void; onUpload: (file?: File) => void; onTransition: (action: "submit" | "approve" | "publish" | "withdraw" | "correct") => void; onDelete: () => void; onAudit: () => void; onPreview: () => void; onSearchPreview: () => void; onReprocess: () => void }) {
  const status = item.status || "draft"
  return <div className="space-y-3 rounded-md border p-4"><div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><FileText className="h-4 w-4" /><span className="font-medium">{item.title || "Untitled document"}</span><Badge variant="outline">{status}</Badge><Badge variant="secondary">{label(item.document_kind)}</Badge><ExtractionBadge status={item.extraction_status} /></div><p className="mt-1 text-sm text-muted-foreground">{[item.issuing_authority, item.document_number, item.version ? `v${item.version}` : "", item.language].filter(Boolean).join(" · ")}</p>{item.original_filename ? <p className="mt-1 text-xs text-muted-foreground">{item.original_filename} · {formatBytes(item.file_size || 0)} · source SHA-256 {item.checksum_sha256?.slice(0, 12)}… · derived {item.derived_content_checksum?.slice(0, 12) || "not available"} · index {label(item.search_index_status)}{item.indexed_at ? ` at ${new Date(item.indexed_at).toLocaleString()}` : ""}</p> : <p className="mt-1 text-xs text-amber-700">A managed file is required before review.</p>}{status === "published" && item.search_index_status !== "indexed" ? <p className="mt-2 flex items-center gap-1 text-sm text-amber-700"><AlertTriangle className="h-4 w-4" />Published document is not searchable. Review extraction errors and reprocess it.</p> : null}{item.extraction_error ? <p className="mt-2 text-sm text-destructive">Extraction error: {item.extraction_error}</p> : null}</div><div className="flex flex-wrap gap-2"><Button size="sm" variant="ghost" disabled={busy} onClick={onAudit}><History className="mr-1 h-4 w-4" />Audit</Button>{item.supports_preview ? <Button size="sm" variant="outline" disabled={busy} onClick={onPreview}><Eye className="mr-1 h-4 w-4" />Derived preview</Button> : null}<Button size="sm" variant="outline" disabled={busy || !item.original_filename} onClick={onSearchPreview}><FileSearch className="mr-1 h-4 w-4" />Search preview</Button>{item.original_filename ? <Button size="sm" variant="outline" disabled={busy} onClick={onReprocess}><RefreshCw className="mr-1 h-4 w-4" />Reprocess</Button> : null}{status === "draft" ? <><Button size="sm" variant="outline" disabled={busy} onClick={onEdit}><Pencil className="mr-1 h-4 w-4" />Edit metadata</Button><label className="inline-flex h-9 cursor-pointer items-center rounded-md border px-3 text-sm"><FileUp className="mr-1 h-4 w-4" />{item.original_filename ? "Replace original" : "Upload original"}<input className="sr-only" type="file" accept=".pdf,.docx,.xlsx,.md,.txt" disabled={busy} onChange={event => { onUpload(event.target.files?.[0]); event.currentTarget.value = "" }} /></label><Button size="sm" variant="destructive" disabled={busy} onClick={onDelete}><Trash2 className="h-4 w-4" /><span className="sr-only">Delete</span></Button></> : null}{status === "published" && item.id ? <Button size="sm" variant="outline" asChild><a href={`/api/public/outbreaks/${item.outbreak_id}/documents/${item.id}/download`} target="_blank" rel="noreferrer"><Download className="mr-1 h-4 w-4" />Original</a></Button> : null}</div></div><div className="flex flex-wrap gap-2">{status === "draft" ? <Button size="sm" variant="outline" disabled={busy || !item.original_filename} onClick={() => onTransition("submit")}>Submit for clinical review</Button> : null}{status === "pending_review" && !item.approved_at ? <Button size="sm" variant="outline" disabled={busy} onClick={() => onTransition("approve")}><CheckCircle2 className="mr-1 h-4 w-4" />Clinician approve</Button> : null}{status === "pending_review" && item.approved_at ? <Button size="sm" disabled={busy} onClick={() => onTransition("publish")}>Publish</Button> : null}{status === "published" ? <Button size="sm" variant="outline" disabled={busy} onClick={() => onTransition("correct")}>Create correction</Button> : null}{status !== "draft" && status !== "withdrawn" ? <Button size="sm" variant="destructive" disabled={busy} onClick={() => onTransition("withdraw")}>Withdraw</Button> : null}</div></div>
}

function ExtractionBadge({ status }: { status?: string }) { const value = status || "not_available"; const variant = value === "ready" ? "default" : value === "failed" ? "destructive" : "outline"; return <Badge variant={variant}>{value === "ready" ? "Searchable" : value === "failed" ? "Extraction failed" : "Original only"}</Badge> }
