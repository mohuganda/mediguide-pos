...
describe("typed calculator and drug services", () => {
  ...
  it("serializes calculator list filters as typed query parameters", async () => {
    const fetchMock = vi.fn().mockResolvedValue(json({
      items: [], page: 1, per_page: 20, total_items: 0, total_pages: 0,
    }))
    vi.stubGlobal("fetch", fetchMock)

    await calculatorService.listTable({ page: 1, perPage: 20, search: "triage", filters: [] }, "decision_tool")
  ...
  it("maps legacy drug form fields to the typed drug DTO", async () => {
    const fetchMock = vi.fn().mockResolvedValue(json({
      id: "drug-1", name: "Example", status: "active", review_status: "approved",
    }, 201))
    vi.stubGlobal("fetch", fetchMock)
