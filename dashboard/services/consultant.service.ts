...
export const consultantService = {
  async list(
    ...
    const result = await backend().request<ConsultantPageEnvelope>(
      "/api/v2/consultants",
      { query: params },
    );
  ...
  ): Promise<DomainPageResult<Consultant>> {
  ...
  async get(id: string) {
    const result = await backend().request<ConsultantEnvelope>(
  ...
  async update(id: string, data: ConsultantWrite) {
    const result = await backend().request<ConsultantEnvelope>(
      `/api/v2/consultants/${id}`,
      { method: "PATCH", body: JSON.stringify(payload(data)) },
    );
    return normalized(result.item);
  },

  delete(id: string) {
    return backend().request<void>(`/api/v2/consultants/${id}`, {
      method: "DELETE",
    });
  },
};
