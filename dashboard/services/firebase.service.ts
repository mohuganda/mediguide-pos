...
export type TestPushDeviceResult = {
  device_id: string;
  platform: string;
  app_version?: string;
  state: "validated" | "accepted" | "rejected";
...
export type TestPushResult = {
  ...
  accepted: number;
  failed: number;
  devices: TestPushDeviceResult[];
};
export type FirebaseTestRecipient = {
  id: string;
  name: string;
  email: string;
  device_count: number;
...
export const firebaseService = {
  ...
  remoteConfig() {
    return client().send<RemoteConfigDocument>(
  ...
  searchTestRecipients(search: string) {
    return client().send<FirebaseTestRecipient[]>(
