...
- Mobile converts remaining transitional filter expressions into allowlisted
  query parameters. Notifications now use explicit query parameters and a
  focused repository.
...
| Notifications — migrated | Former compatibility collections: `notifications`, `notification_templates`, `notification_campaigns` | Dashboard notices and campaign settings use `notificationsService`; mobile lists through `NotificationRepository`. Search, type, priority, read-state, date, pagination and allowlisted sorting execute in PostgreSQL. | Administrators publish notices and manage templates/campaigns; users own read receipts and read-all operations. | Typed `/api/v2/notifications`, `/api/v2/notifications/{id}`, `/read`, `/unread`, `/read-all`, `/api/v2/notification-templates`, and `/api/v2/notification-campaigns`. | A user sees only global or directly addressed notices and can mutate only their own read receipt; `admin.all` is required for publication, templates and campaigns. |
...
| Reference and content data | `languages`, `settings`, `generic_pages`, `ministry_directory` | Dashboard pages/localization/settings/directory; mobile language, generic viewer and ministry directory. Search, active/status filters and locale lookup. | Admin/editor CRUD; translation/content payload download where required. | Typed `/api/v2/languages` and `/api/v2/settings`; transitional `/api/v2/pages` and `/api/v2/ministry-directory`. | Published read; relevant editor/admin write. |

## Recommended migration order

Calculators, decision tools, drugs, users/roles/permissions, facilities with
geographic reference data, and support/help content are complete. The next
required slice is consultants. The remaining guideline
taxonomy and legacy medical-guideline records then need dedicated contracts
around the already typed Markdown and publishing workflow.
